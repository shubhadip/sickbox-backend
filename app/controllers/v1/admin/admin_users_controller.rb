class V1::Admin::AdminUsersController < ApplicationController  # V1::AdminController
    ...
    def index
      ...
      @admin_users = @admin_users.where(enable:true) if params["status"] == "enabled"
    ...
    def get_static_data
     ...
     APP_CONFIG["departments"].each do |department_id, department|    
      department.map{|department_name, designations| data[:departments] << {id: department_id, name: department_name, designation: designations}}
     end
     render json: data
    end
    ...
    private
      ...
      def admin_user_params
        params.require(:admin_user).permit(:first_name, :last_name, :email, :personal_email, :password, :designation, :department, :enable, :can_login, :can_sales_login, :mobile, :mobile_other, :office_location, :employee_code)
      end
      ...
      def set_fields
        ...
        @fields = (all_fields & AdminUser.attribute_names) | [:id]
        @additional_fields = (all_fields & AdminUser.reflect_on_all_associations.map {|object| object.name.to_s})
      end
  end
