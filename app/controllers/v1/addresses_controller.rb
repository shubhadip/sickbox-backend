class V1::AddressesController < ApiController
  ...
  before_action :set_address, only: %i[show update destroy] #:pg_options
  before_action :set_fields, only: %i[show index]
  # before_action :check_pincode, only: [:show, ] #:pg_options
  before_action :check_address_validation, only: %i[update create]

  # GET /v1/addresses
  # GET /v1/addresses.json
  def index
    @addresses =
      Address.active.where(user_id: current_user.id).order('status desc')
  end
  ...
  def update
    if (
         params[:address][:status] == 'default' ||
           params[:address][:status] == 'disabled'
       ) &&
         params[:address].length == 1
