module Authentication
    ...
    included do
     ...
     module ClassMethods
      def check_inactive(user)
        is_inactive = (user.status == "inactive") if self.name.underscore == 'user'
    ...
    def authenticate(authentication_params = nil)
      ...
      condition ||= (self.encrypted_password == class_object.encrypt_password(password) || self.encrypted_password == class_object.encrypt_password(Rails.application.secrets.encrption_key+password))
      condition ||= (authentication_params[:otp_code].present? and verify_otp(authentication_params))
      ...
      if condition and condition_can_login
        ...
        expire_time = APP_CONFIG['session_expiry'][authentication_params[:device]] || APP_CONFIG['session_expiry']['default']
    ...
    def user_show(token=nil)
      ...
      @user_show  = self.attributes.symbolize_keys.slice(:id, :first_name, :last_name, :gender, :date_of_birth, :email, :can_sales_login, :mobile, :non_promotional_wallet_amount, :is_verified)
      ...
      if trials > 2
        ...
        if (APP_CONFIG["sms"]["no_otp_verification"].exclude? params["mobile"])
          ...
          if resend_value > 3
            ...
            Sidekiq::Client.push('class'=>QueueSendSms, 'queue'=>'otp', 'args'=>["otp", {"otp" => otp}, params["mobile"]])
            return true
          else
    ...
    def check_login_attempts(login_condition)
      ...
      if login_attempt > threshold_attempts
        ...
        if login_condition
          ...
          $redis.incr(key)
          $redis.expire(key, 14400) if login_attempt == 0
