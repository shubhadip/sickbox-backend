class Address < ApplicationRecord
  ...
  has_many :orders
  #enums
  enum status: %i[disabled enabled default]
  ...
  validates :alternate_mobile,
            length: { is: 10 },
            if: lambda { |address|
              address.alternate_mobile.present? and
                address.alternate_mobile_changed?
            }
  #callbacks
  after_update :update_status,
               if: lambda { |address|
                 address.status_changed? and address.status == 'default'
               }
  before_save :check_address

  def full_address
    "#{first_name} #{last_name} <br/> #{address} <br/> #{landmark} <br/> #{
      city
    }-#{pincode} <br/> #{state} <br/> #{country}"
  end
  ...
  def update_status
    Address.active.where(user_id: self.user_id).where.not(id: self.id)
      .update_all({ status: 1 })
  end
